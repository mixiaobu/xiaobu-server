...
**一套“起步即规模化”的 Kotlin 微服务底座。**  
以 Spring Boot 4 / Spring Cloud 2025 为主引擎，集成注册发现、配置中心、统一网关、OAuth2 授权与用户中心等核心能力，并用 `common` 模块把工程通用能力打包成“即插即用”的能力集合。**能快速起盘、也能稳态扩展**，是企业级后端体系从 0 到 1、从 1 到 N 的高效支点。
...
- **config**：统一配置中心（Spring Cloud Config Server），集中化配置治理。默认端口 `8081`。
- **gateway**：统一网关入口，负责路由与转发。默认端口 `8088`。
- **oauth**：OAuth2 授权服务器，支持自定义密码模式（Password Grant）扩展。默认端口 `8082`。
...
- **企业级中台/平台化建设**：快速搭起统一网关 + 权限体系 + 用户中心的基座。
- **多团队并行研发**：微服务拆分明确、公共能力统一复用，降低重复劳动。
- **需要可持续演进的系统**：配置中心、资源服务、权限加载路径清晰，便于长期维护与扩展。